...
class ComplaintResponseForm(forms.Form):
    ...
    internal_notes = forms.CharField(
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 3,
            'placeholder': 'Внутренние заметки для команды модерации...'
        }),
        required=False,
        label='Внутренние заметки'
    )
    
    # Action types for moderator
    ACTION_CHOICES = [
        ('', 'Без дополнительных действий'),
        ('temporary_ban', 'Временная блокировка пользователя'),
        ('permanent_ban', 'Постоянная блокировка пользователя'),
    ...
    action_reason = forms.CharField(
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 2,
            'placeholder': 'Причина дополнительного действия...'
        }),
        required=False,
