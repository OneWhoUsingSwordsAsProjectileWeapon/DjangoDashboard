...
from .models import ReportCategory, Report, BannedUser, ForbiddenKeyword, ListingApproval, ModerationLog, UserComplaint
...
class UserComplaintAdmin(admin.ModelAdmin):
    list_display = ['id', 'complainant', 'complaint_type', 'status', 'priority', 'assigned_moderator', 'created_at']
    list_filter = ['complaint_type', 'status', 'priority', 'created_at']
    search_fields = ['complainant__username', 'subject', 'description']
    readonly_fields = ['created_at', 'updated_at']
    fieldsets = (
        ('Complaint Details', {
            'fields': ('complainant', 'complaint_type', 'subject', 'description')
        }),
        ('Target', {
            'fields': ('target_user', 'target_listing', 'target_booking')
        }),
        ('Status & Assignment', {
            'fields': ('status', 'priority', 'assigned_moderator')
        }),
        ('Resolution', {
            'fields': ('moderator_notes', 'resolution_notes', 'resolved_at')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at')
        }),
