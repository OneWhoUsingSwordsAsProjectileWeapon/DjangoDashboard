...
class Command(BaseCommand):
    ...
    def create_subscription_logs(self, subscription, now):
        """Create logs for subscription lifecycle"""
        logs = []
        ...
        logs.append(SubscriptionLog.objects.create(
            ...
            action='created',
            description=f'Subscription created for plan {subscription.plan.name}',
            performed_at=subscription.start_date
        ))
        ...
        if subscription.status != 'pending':
            logs.append(SubscriptionLog.objects.create(
                ...
                description=f'Payment of ${subscription.amount_paid} processed',
                performed_at=subscription.start_date
            ))

        # Status change logs
        if subscription.status == 'canceled':
            cancel_date = subscription.start_date + timedelta(
                days=random.randint(1, (subscription.end_date - subscription.start_date).days)
            )
            logs.append(SubscriptionLog.objects.create(
                subscription=subscription,
                action='canceled',
                description='Subscription canceled by user',
                performed_at=cancel_date
            ))
            ...
            logs.append(SubscriptionLog.objects.create(
                ...
                performed_at=subscription.end_date
            ))

        # Random events
        if random.random() < 0.1:  # 10% chance of auto-renew toggle
            toggle_date = subscription.start_date + timedelta(
                days=random.randint(1, min(30, (subscription.end_date - subscription.start_date).days))
            )
            logs.append(SubscriptionLog.objects.create(
                subscription=subscription,
                action='auto_renew_toggled',
                description=f'Auto-renew {"enabled" if subscription.auto_renew else "disabled"}',
                performed_at=toggle_date
            ))
